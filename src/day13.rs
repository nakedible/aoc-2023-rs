...
pub fn puzzle1(filename: &str) -> Result<i64> {
    ...
    let mut total = 0;
    for m in input {
        let count = (find_mirror(&m).unwrap_or(0) * 100) + find_mirror(&m.rotated_cw(1)).unwrap_or(0);
        total += count as i64;
    }
    Ok(total)
}
...
fn find_mirror_smudged(input: &Matrix<bool>) -> Option<usize> {
    for y in 1..input.rows {
        ...
        for diff in 1..input.rows {
            ...
            match (0..input.columns).filter(|x| input[(y + diff - 1, *x)] != input[(y - diff, *x)]).count() {
                0 => (),
                1 if smudged => { break; }
                1 => { smudged = true; }
                _ => { break; }
...
pub fn puzzle2(filename: &str) -> Result<i64> {
    ...
    let mut total = 0;
    for m in input {
        let count = (find_mirror_smudged(&m).unwrap_or(0) * 100) + find_mirror_smudged(&m.rotated_cw(1)).unwrap_or(0);
        total += count as i64;
    }
    Ok(total)
}
